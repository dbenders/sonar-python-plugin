 ...
 * @since 2.4
 */
public final class NoseUtils {

	static final String NOSE_REPORT_PATH_PROPERTY = "sonar.nose.reportPath";
	public static final String NOSE_REPORT_FILENAME = "nosetests.xml";
...
//    	}
		...
		if (report == null || !report.exists() || !report.isFile()) {
			Logs.INFO.warn("Nosetests report not found at {}", report);
			report = null;
		}
		return report;
	}
	...
	private static File getReportFromProperty(Project project) {
		String path = (String) project.getProperty(NOSE_REPORT_PATH_PROPERTY);
		if (path != null) {
			return new File(project.getFileSystem().resolvePath(path), NOSE_REPORT_FILENAME);
		}
		return null;
	}
...
//  }
	...
	private static File getReportFromDefaultPath(Project project) {
		return new File(project.getFileSystem().getBasedir(), NOSE_REPORT_FILENAME);
	}
