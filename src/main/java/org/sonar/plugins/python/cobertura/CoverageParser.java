...
public class CoverageParser {
	private static final Logger LOG = LoggerFactory.getLogger(CoverageParser.class);
	private ProjectFileSystem fileSystem;
	
	public CoverageParser(Project project) {
	...
	private void collectPackageMeasures(SMInputCursor pack, SensorContext context) throws ParseException, XMLStreamException {
		while (pack.getNext() != null) {
			...
			for (FileData cci : fileDataPerFilename.values()) {
				LoggerFactory.getLogger(CoverageSensor.class).info("file {} exists {}", cci.getFile(), isFileExist(context, cci.getFile()));
	...
	private void collectFileMeasures(SMInputCursor clazz, Map<String, FileData> dataPerFilename) throws ParseException, XMLStreamException {
		while (clazz.getNext() != null) {
			String fileName = CoverageUtils.relativizePath(fileSystem, clazz.getAttrValue("filename"));
